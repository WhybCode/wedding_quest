...
const CONFIG = {
  ...
  locations: [
    ...
    },
    {
      ...
      addr: "Jakubské náměstí 11, 602 00 Brno",
...
// ============ SITE VARIANT (full invitation vs afterparty) ============
const VARIANT_CONFIGS = buildVariantConfig(CONFIG.maps);

type Guest = { id: string; name: string; attending: boolean };

/** Aktívny variant — nastaví sa pri mounte WeddingQuest (naraz je len jedna stránka). */
let activeVariant: SiteVariant = "full";
let SECTIONS: SectionDef[] = VARIANT_CONFIGS.full.sections;
let SECTION_IDS: string[] = [...VARIANT_CONFIGS.full.sectionIds];
let PROGRAM: ProgramStop[] = VARIANT_CONFIGS.full.program;
let PROGRAM_TRAIL_STOPS: TrailPoint[] = VARIANT_CONFIGS.full.programTrailStops;
let PROGRAM_TRAIL_BULGES: number[] = VARIANT_CONFIGS.full.programTrailBulges;
let PROGRAM_TRAIL_WIDE = VARIANT_CONFIGS.full.programTrailWide;
let PROGRAM_TRAIL_LIFT = VARIANT_CONFIGS.full.programTrailLift;
let PROGRAM_TRAIL_SEGMENT_COUNT = Math.max(1, PROGRAM_TRAIL_STOPS.length - 1);
let GUEST_KEY = VARIANT_CONFIGS.full.storage.guests;
let RSVP_SENT_KEY = VARIANT_CONFIGS.full.storage.rsvpSent;
let POKRM_SENT_KEY = VARIANT_CONFIGS.full.storage.pokrmSent;
let POKRM_DATA_KEY = VARIANT_CONFIGS.full.storage.pokrmData;
let UBYTOVANIE_SENT_KEY = VARIANT_CONFIGS.full.storage.ubytovanieSent;
let UBYTOVANIE_DATA_KEY = VARIANT_CONFIGS.full.storage.ubytovanieData;
let KONTAKT_SENT_KEY = VARIANT_CONFIGS.full.storage.kontaktSent;
let KONTAKT_DATA_KEY = VARIANT_CONFIGS.full.storage.kontaktData;
let CHECK_KEY = VARIANT_CONFIGS.full.storage.checked;
let ACHIEVEMENT_KEY = VARIANT_CONFIGS.full.storage.achievement;
let FORM_PREFIX = VARIANT_CONFIGS.full.formPrefix;
let showRsvp = VARIANT_CONFIGS.full.showRsvp;
let showPokrm = VARIANT_CONFIGS.full.showPokrm;
let showDresscode = VARIANT_CONFIGS.full.showDresscode;
let showFaq = VARIANT_CONFIGS.full.showFaq;
let hotelOptional = VARIANT_CONFIGS.full.hotelOptional;
let LEVELS = VARIANT_CONFIGS.full.levels;
let STICKMAN_NAV_IDS: string[] = [...SECTION_IDS, "footer"];

function applySiteVariant(variant: SiteVariant) {
  const cfg = VARIANT_CONFIGS[variant];
  activeVariant = variant;
  SECTIONS = cfg.sections;
  SECTION_IDS = [...cfg.sectionIds];
  PROGRAM = cfg.program;
  PROGRAM_TRAIL_STOPS = cfg.programTrailStops;
  PROGRAM_TRAIL_BULGES = cfg.programTrailBulges;
  PROGRAM_TRAIL_WIDE = cfg.programTrailWide;
  PROGRAM_TRAIL_LIFT = cfg.programTrailLift;
  PROGRAM_TRAIL_SEGMENT_COUNT = Math.max(1, PROGRAM_TRAIL_STOPS.length - 1);
  GUEST_KEY = cfg.storage.guests;
  RSVP_SENT_KEY = cfg.storage.rsvpSent;
  POKRM_SENT_KEY = cfg.storage.pokrmSent;
  POKRM_DATA_KEY = cfg.storage.pokrmData;
  UBYTOVANIE_SENT_KEY = cfg.storage.ubytovanieSent;
  UBYTOVANIE_DATA_KEY = cfg.storage.ubytovanieData;
  KONTAKT_SENT_KEY = cfg.storage.kontaktSent;
  KONTAKT_DATA_KEY = cfg.storage.kontaktData;
  CHECK_KEY = cfg.storage.checked;
  ACHIEVEMENT_KEY = cfg.storage.achievement;
  FORM_PREFIX = cfg.formPrefix;
  showRsvp = cfg.showRsvp;
  showPokrm = cfg.showPokrm;
  showDresscode = cfg.showDresscode;
  showFaq = cfg.showFaq;
  hotelOptional = cfg.hotelOptional;
  LEVELS = cfg.levels;
  STICKMAN_NAV_IDS = [...SECTION_IDS, "footer"];
  CHECKLIST_IDS.clear();
  for (const s of SECTIONS) CHECKLIST_IDS.add(s.id);
}
...
export function WeddingQuest({ variant = "full" }: { variant?: SiteVariant }) {
  applySiteVariant(variant);

  const [checked, setChecked] = useState<Set<string>>(new Set());
  const [active, setActive] = useState<string>("hero");
  const [mobileOpen, setMobileOpen] = useState(false);
  const [desktopChecklistOpen, setDesktopChecklistOpen] = useState(true);
  ...
  useEffect(() => {
    ...
    sync();
    try {
      if (showRsvp && window.localStorage.getItem(RSVP_SENT_KEY) === "1") markSectionChecked("rsvp");
      if (showPokrm && window.localStorage.getItem(POKRM_SENT_KEY) === "1") markSectionChecked("pokrm");
      if (!hotelOptional && window.localStorage.getItem(UBYTOVANIE_SENT_KEY) === "1") markSectionChecked("ubytovanie");
  ...
  return (
    <div className="relative min-h-screen overflow-x-visible">
      ...
      <div className="site-body relative">
        ...
        <ProgramSection />
        <LokacieSection />
        {showRsvp ? <RsvpSection /> : null}
        <UbytovanieSection />
        {showPokrm ? <PokrmSection /> : null}
        {showDresscode ? <DresscodeSection /> : null}
        <DarySection />
        <DenSection />
        <BrnoSection />
        <FotkySection />
      ...
      <DesktopChecklist
        open={desktopChecklistOpen}
        onToggle={() => setDesktopChecklistOpen((o) => !o)}
        checked={checked}
        active={active}
        onPick={scrollTo}
        onToggleCheck={toggleCheck}
...
function DesktopChecklist({
  open, onToggle, checked, active, onPick, onToggleCheck, onClearProgress, progress, questComplete,
}: {
  open: boolean;
  onToggle: () => void;
  ...
  return (
    <aside
      className={`fixed right-6 top-6 z-30 hidden w-[320px] overflow-visible lg:block ${
        open ? "bottom-6" : ""
      }`}
    >
      <div
        className={`paper-card notebook-lines relative flex flex-col overflow-hidden p-5 pt-8 ${
          open ? "h-full" : ""
        }`}
      >
        <div className="tape" />

        <div className="mb-2 flex shrink-0 items-center gap-2 border-b-2 border-dashed border-[color:var(--ink)]/20 pb-2">
        ...
        {open ? (
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
            <div className="mb-3 h-2 shrink-0 overflow-hidden rounded-full bg-[color:var(--ink)]/10">
              <div className="h-full rounded-full bg-[color:var(--turquoise)] transition-all" style={{ width: `${progress}%` }} />
            </div>
            <p className="mb-2 shrink-0 font-hand text-xs text-[color:var(--ink)]/60">Odčiarkni si sám, keď máš hotovo ✍️</p>
            ...
            <div className="relative mt-auto shrink-0 pt-1">
              <StickmanControlHintsDesktop />
            </div>
          </div>
        ) : (
          <div className="mb-1 h-1.5 overflow-hidden rounded-full bg-[color:var(--ink)]/10">
            <div className="h-full rounded-full bg-[color:var(--turquoise)] transition-all" style={{ width: `${progress}%` }} />
          </div>
        )}

        <div className="relative -mx-5 -mb-5 mt-1 flex shrink-0 items-center justify-center py-1.5">
          <button
            type="button"
            onClick={onToggle}
            className="flex items-center justify-center text-[color:var(--ink)]/35 transition hover:text-[color:var(--bordo)]"
            aria-expanded={open}
            aria-label={open ? "Skryť quest log" : "Zobraziť quest log"}
            title={open ? "Skryť quest log" : "Zobraziť quest log"}
          >
            <ChevronDown
          ...
          {open && checked.size > 0 ? (
            <button
              ...
              className="absolute right-5 -translate-y-1 rounded p-0.5 text-[color:var(--ink)]/25 transition hover:text-[color:var(--ink)]/55"
...
// ============ Section wrapper ============
function Section({
  id, level, title, children, className = "",
}: { id: string; level: string; title: string; children: React.ReactNode; className?: string }) {
  ...
  return (
    <section
      id={id}
      className={`scroll-mt-[4.25rem] py-16 pb-24 lg:scroll-mt-4 lg:py-24 lg:pb-32 ${className}`.trim()}
    >
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ChevronDown className={`h-3 w-3 transition-transform ${shown ? "rotate-180" : ""}`} />
      </button>
      {shown ? (
        <div className="absolute left-0 right-0 top-full z-50 pt-1" role="menu">
...
function HeroSection() {
  ...
  const friends = activeVariant === "afterparty";
      ...
      <div className="grid gap-8 lg:grid-cols-[1.3fr_1fr] items-center">
        <div>
          <p className="font-hand text-2xl text-[color:var(--paper)] mb-2">
            {friends
              ? "Zdá sa, že ste pozvaní na afterku."
          ...
          <p className="max-w-xl text-lg text-[color:var(--paper)]/80 leading-relaxed">
            {friends ? (
              <>
                Hostinu nechávame v užšiom kruhu, ale večer otvárame — a chceme ťa pri tom!
                Príď na obrad o 15:30, večer sa stretneme na afterke približne od 21:00. Nižšie nájdeš program,
                lokácie a pár praktických vecí. Ak máš záujem o hotel, rozklikni side quest.
              </>
            ) : (
              <>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {friends ? (
              ...
              >
                <Heart className="h-4 w-4" /> Potvrdzujem afterku
              </a>
            ) : (
              <a
                href="#rsvp"
...
// ============ PROGRAM (data z applySiteVariant) ============
...
function ProgramTrail() {
  ...
  return (
    <div
      className={`program-trail${activeVariant === "afterparty" ? " program-trail--afterparty" : ""}`}
      data-stickman-rail="program-trail"
    >
      <svg
      ...
      {PROGRAM.map((p, i) => {
        ...
        return (
          ...
          >
            ...
            <div className={`program-trail-label${isWide ? " program-trail-label--wide" : ""}${activeVariant === "afterparty" ? " program-trail-label--fit" : ""}`}>
...
function ProgramSection() {
  ...
  return (
    <Section
      id="program"
      level={levelFor("program")}
      title={friends ? "Program večera" : "Program dňa"}
      className={friends ? "!pb-10 lg:!pb-14" : ""}
    >
      <p className="font-hand text-2xl text-[color:var(--gold)] mb-8 max-w-[50.4rem] leading-relaxed">
        {friends
          ? "Obrad o 15:30, potom afterka od 21:00 — raut o 22:00 a dozvuky do noci. Tu je skrátená mapa checkpointov."
          : 'Náš Wedding Quest s krycím menom "10.10.2026" má viacero levelov a tie zase svoje checkpointy. Tu je ich zoznam a mapa, aby ste neskončili pri vedľajšej misii v zlej pivnici či v skrytom leveli.'}
      </p>
      <ProgramTrail />
    </Section>
  );
...
function LokacieSection() {
  ...
  const locations = CONFIG.locations.map((l) => {
    if (!friends) return l;
    if (l.id === "kumst") {
      return { ...l, desc: "Afterka, raut a dozvuky. Tu sa stretneme večer." };
    }
    if (l.id === "hotel") {
      return { ...l, desc: "Voliteľné ubytovanie — side quest nižšie, ak máš záujem." };
    }
    return l;
  });
...
async function submitForm(formName: string, data: Record<string, unknown>) {
  ...
  // Afterparty aj full idú do rovnakých sheet formulárov (rsvp/ubytovanie/poznamka…).
  const formKey = `${FORM_PREFIX}${formName}`;
  const payload = {
    _subject: `Svadba · ${formKey}${subjectSuffix}`,
    _form: formKey,
...
function RsvpSection() {
  ...
  useEffect(() => {
    try {
      ...
      if (stored.length > 0) {
        ...
        setName(main.name);
        if (!afterparty) setAttending(main.attending ? "yes" : "no");
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const isAttending = afterparty || attending === "yes";
    ...
    const r = await submitForm("rsvp", {
      ...
      name,
      phone,
      attending: afterparty ? "afterparty" : attending,
      guests: list,
  ...
  if (sent) {
    return (
      <Section id="rsvp" level={levelFor("rsvp")} title="RSVP aneb prezenčka">
  ...
  return (
    <Section id="rsvp" level={levelFor("rsvp")} title="RSVP aneb prezenčka">
      <p className="font-hand text-2xl text-[color:var(--gold)] mb-8 max-w-2xl">
        {afterparty
          ? "Potvrď prosím, že ťa uvidíme na afterke — bez toho sa quest log neuloží."
...
function PokrmSection() {
  ...
  return (
    <Section id="pokrm" level={levelFor("pokrm")} title="Power-up pokrm">
      {guests.length === 0 ? (
...
function UbytovanieSection() {
  const guests = useGuests().filter((g) => g.attending);
  const [expanded, setExpanded] = useState(!hotelOptional);
  ...
  useEffect(() => {
    try {
      if (window.localStorage.getItem(UBYTOVANIE_SENT_KEY) === "1") {
        setSent(true);
        if (hotelOptional) setExpanded(true);
      }
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const contactPerson =
      guests.find((g) => g.id === rooms[0]?.mainGuestId)?.name?.trim() || "";
    setLoading(true);
    const r = await submitForm("ubytovanie", {
      rooms: payload,
      totalPrice: total,
      night: "10.10.2026 → 11.10.2026",
      mainPerson: contactPerson,
      hp: fd.get("hp"),
    });
    setLoading(false);
    if (r.ok) {
      ...
      if (!hotelOptional) markSectionChecked("ubytovanie");
        ...
        {rooms.map((r, idx) => (
          <div key={r.id} className="rounded-lg border-2 border-dashed border-[color:var(--ink)]/30 bg-white/40 p-4">
            <div className="mb-3 flex items-baseline justify-between gap-2">
              <h3 className="font-display text-xl text-[color:var(--bordo)]">
                Izba #{idx + 1}
                {idx === 0 ? (
                  <span className="ml-2 font-hand text-sm font-normal text-[color:var(--ink)]/55">
                    (kontaktná osoba = hlavná osoba)
                  </span>
                ) : null}
              </h3>
              <button
    ...
    <Section id="ubytovanie" level={levelFor("ubytovanie")} title="Hotel ala čik-čik domček">
      ...
      {hotelOptional && !expanded && !sent ? (
        <button
          ...
          className="paper-card flex w-full items-center justify-between gap-4 p-5 text-left transition hover:bg-white/50"
        >
          <div>
            <p className="font-marker text-xs uppercase tracking-wide text-[color:var(--bordo)]">Voliteľné</p>
            <p className="mt-1 font-hand text-xl text-[color:var(--ink)]">
              Mám záujem o hotel — rozkliknúť formulár
            </p>
          </div>
          <ChevronDown className="h-6 w-6 shrink-0 text-[color:var(--bordo)]" />
        </button>
      ) : (
        <>
          {hotelOptional && expanded && !sent ? (
            <button
              ...
              className="mb-4 inline-flex items-center gap-2 font-hand text-base text-[color:var(--paper)]/70 hover:text-[color:var(--gold)]"
            >
              <ChevronRight className="h-4 w-4 rotate-[-90deg]" /> Skryť formulár
            </button>
          ) : null}
          {formBody}
        </>
...
function DresscodeSection() {
  return (
    <Section id="dresscode" level={levelFor("dresscode")} title="Dresscode a.k.a. skin pack">
      <p className="font-hand text-2xl text-[color:var(--gold)] mb-8 max-w-[50.4rem] leading-relaxed">
      Definovaný heslom „elegantne ale hravo“. Očakávame, že prídeš nahodený/á slávnostne, ale najmä pohodlne na celý deň slávenia.
      Farby a štýl necháme na teba, pokojne môžeš ladiť s našou bordovo-jesenno-pestrofarebnou paletou a tu sa inšpirovať:
      </p>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="paper-card p-6">
          ...
          <ul className="space-y-2 font-hand text-lg text-[color:var(--ink)]">
            ...
            <li>· Jednofarebné, nevzorované odevy podporujeme</li>
            <li>· Náhradný gombík na nohavice, pre prípad, že pôvodný odstrelí</li>
          </ul>
        </div>
        <div className="paper-card p-6">
...
function DarySection() {
  return (
    <Section id="dary" level={levelFor("dary")} title="Loot & kvety">
...
function DenSection() {
  return (
    <Section id="den" level={levelFor("den")} title="Lov na XP">
...
const BRNO_TIPS = [
  ...
  { title: "Famózna prežieračka", desc: "ZAZA; 3F Sushi; Ramen Brno; Špageta; Padagali; Bango Brno; Paprika" },
  ...
  { title: "Pivné bruško podporia", desc: "U Tekutého Chleba; Pivní Burza; Pivní Palác; Na Stojáka; Pivovar HARRY; Lokál U Caipla; U Tomana" },
...
function BrnoSection() {
  ...
  return (
    <Section id="brno" level={levelFor("brno")} title="Brno DLC">
...
const FAQ: FaqItem[] = [
  ...
  { q: "Koľko vychádza hotel na noc za jednu osobu ?", a: "Hradí sa cena na izbu, tak ako je uvedená vo formulári - necháme na vás ako si to podelíte podľa lôžok, medzi možnosťami je i jednolôžková izba." },
  {
    ...
    a: [
      "Prečo nie?",
      "Lebo medveď...",
      "Pretože mladomanželia sa tak rozhodli.",
...
function pickFaqAnswer(a: string | string[], last?: string) {
  ...
  const pool = last ? a.filter((x) => x !== last) : a;
  const options = pool.length > 0 ? pool : a;
  return options[Math.floor(Math.random() * options.length)] ?? a[0];
}
...
function FaqSection() {
  ...
  const lastAnswerByQ = useRef<Record<string, string>>({});

  function ask(item: FaqItem) {
    const answer = pickFaqAnswer(item.a, lastAnswerByQ.current[item.q]);
    lastAnswerByQ.current[item.q] = answer;
    setMessages((m) => [...m, { role: "user", text: item.q }, { role: "bot", text: answer }]);
    setTimeout(() => {
  ...
  return (
    <Section id="faq" level={levelFor("faq")} title="Svadobný NPC poradca">
...
function KontaktSection() {
  ...
  return (
    ...
    >
      ...
      <p className="font-hand text-2xl text-[color:var(--gold)] mb-8 max-w-[50.4rem] leading-relaxed">
        {activeVariant === "afterparty"
          ? "Si na afterparty linku — ak niečo nevieš nájsť, ozvi sa nám. Máš tip, diétu alebo iný detail? Sem s poznámkou."
            ...
            {!rsvpName ? (
              <div>
                ...
                <p className="mt-1 font-hand text-sm text-[color:var(--ink)]/55">
                  {activeVariant === "afterparty"
                    ? "Napíš meno, aby sme vedeli, od koho poznámka je."
      ...
      <footer id="quest-footer" className="mt-auto pb-6 text-center">
        ...
        <p className="mt-2 font-hand text-lg text-[color:var(--paper)]/60">
          {activeVariant === "afterparty"
            ? "Afterka unlock · Tešíme sa večer !!"
